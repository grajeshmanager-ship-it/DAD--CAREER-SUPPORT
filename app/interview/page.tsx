...
export default function InterviewPage() {
  ...
  // Camera setup for live interview
  ...
  // Simulated behavior analysis (in production would use face-api.js or similar)
  const startBehaviorAnalysis = () => {
    const fillerWordsList = ["um", "uh", "like", "you know", "basically", "literally", "actually", "so", "right"];
    let wordCount = 0;
    let fillerCount = 0;
    const detectedFillers: string[] = [];

    behaviorIntervalRef.current = setInterval(() => {
      // Simulate real-time behavior scoring
  ...
  // Speech recognition for live interview
  const startSpeechRecognition = () => {
    ...
    recognition.onresult = (event) => {
      ...
      const fillers = words.filter(w => fillerWords.includes(w));
      setBehaviorData(prev => ({
        ...prev,
        fillerCount: prev.fillerCount + fillers.length,
        fillerWords: [...new Set([...prev.fillerWords, ...fillers])],
      }));
    };
  ...
  const handleDebrief = async () => {
    ...
    // Add behavior data to debrief for live mode
    const finalAnswers = mode === "live"
      ? answers.map((a, i) => a || liveTranscript || "(spoken answer)")
      : answers;
  ...
  const startVoiceInterview = async () => {
    ...
    const firstMessage = `Good ${new Date().getHours() < 12 ? "morning" : "afternoon"}. I'm ${prep.interviewerPersona.name}, ${prep.interviewerPersona.title}. Thank you for coming in today. I've reviewed your background and I'd like to start by asking you to tell me about yourself and what draws you to this ${prep.roleTitle} role at ${prep.company}.`;
        ...
        <div style={{ maxWidth: "900px", margin: "0 auto", padding: "80px 52px" }}>
          <div style={{ marginBottom: "64px" }}>
            ...
            <h1 style={{ fontSize: "clamp(32px, 5vw, 60px)", fontWeight: "300", color: text, lineHeight: "1.1", marginBottom: "20px", letterSpacing: "-0.02em" }}>
              ...
              <span style={{ color: "rgba(235,229,220,0.35)" }}>Choose your format.</span>
            </h1>
            <p style={{ fontSize: "15px", color: "rgba(235,229,220,0.38)", lineHeight: "1.9", fontFamily: sans, fontWeight: "300", maxWidth: "520px", margin: 0 }}>
              DAD reads the JD, selects the right interviewer persona, builds the questions, and puts you in the room. You configure nothing.
            </p>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "1px", background: `${blue}15` }}>
            {[
              {
                id: "written" as InterviewMode,
                label: "Written Interview",
                num: "01",
                desc: "Answer questions by typing. Adaptive follow-ups. Full debrief with ideal answers and scoring.",
                tags: ["All roles", "Your own pace", "Deep analysis"],
                color: gold,
              },
              {
                ...
                num: "02",
                desc: "Speak your answers to a realistic AI interviewer. Natural conversation. Professional evaluation.",
                tags: ["Natural flow", "Speaking practice", "Real-time"],
                color: blue,
              },
              {
                id: "live" as InterviewMode,
                label: "Live Simulation",
                num: "03",
                desc: "Camera on. Real interviewer on screen. Behaviour analysis — eye contact, confidence, pace, filler words.",
                tags: ["Camera required", "Behaviour analysis", "Most realistic"],
                color: "#5B9E7A",
                flagship: true,
              },
            ].map((item, i) => (
              <div
                key={i}
                onClick={() => jd.trim().length >= 20 ? handlePrepare(item.id) : setError("Please paste the full job description first.")}
                style={{
                  background: dark, padding: "40px 32px", cursor: "pointer",
              ...
              >
                {item.flagship && (
                  <div style={{ position: "absolute", top: "16px", right: "16px", fontSize: "9px", letterSpacing: "0.12em", textTransform: "uppercase", color: "#5B9E7A", border: "0.5px solid #5B9E7A40", padding: "3px 8px", fontFamily: sans }}>
                    Flagship
                  </div>
                )}
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                  {item.tags.map((tag, j) => (
                    <div key={j} style={{ fontSize: "10px", color: `${item.color}70`, fontFamily: sans, letterSpacing: "0.08em" }}>
                      · {tag}
                    </div>
                  ))}
                </div>
  ...
  // ── INTEL SCREEN (shared by all modes) ──
  if (stage === "intel" && prep) {
      ...
      <div style={{ minHeight: "100vh", background: dark, color: text, fontFamily: serif }}>
        ...
        <div style={{ maxWidth: "900px", margin: "0 auto", padding: "60px 52px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1px", background: `${accent}10`, marginBottom: "32px" }}>
            <div style={{ background: dark, padding: "28px 32px" }}>
              ...
              {prep.whatGetsYouHired.map((w, i) => (
                <div key={i} style={{ fontSize: "13px", color: "rgba(235,229,220,0.45)", fontFamily: sans, marginBottom: "10px", lineHeight: "1.6", display: "flex", gap: "10px" }}>
          ...
          {mode === "live" ? (
            <div>
              {cameraError && (
                <div style={{ padding: "16px 24px", border: "0.5px solid rgba(176,112,112,0.3)", marginBottom: "20px" }}>
                  <p style={{ fontSize: "13px", color: "#B07070", fontFamily: sans, margin: 0 }}>{cameraError}</p>
                </div>
              )}
              <button
                onClick={async () => { await startCamera(); setStage("interview"); setCurrentQ(0); }}
                style={{ width: "100%", background: "#5B9E7A", color: dark, border: "none", padding: "20px", cursor: "pointer", fontSize: "11px", letterSpacing: "0.18em", textTransform: "uppercase", fontFamily: sans }}
              >
            ...
            <button
              onClick={() => { setStage("interview"); setCurrentQ(0); startVoiceInterview(); }}
              style={{ width: "100%", background: blue, color: dark, border: "none", padding: "20px", cursor: "pointer", fontSize: "11px", letterSpacing: "0.18em", textTransform: "uppercase", fontFamily: sans }}
            >
              Begin voice interview →
            </button>
          ) : (
            <button
              onClick={() => { setStage("interview"); setCurrentQ(0); }}
              style={{ width: "100%", background: gold, color: dark, border: "none", padding: "20px", cursor: "pointer", fontSize: "11px", letterSpacing: "0.18em", textTransform: "uppercase", fontFamily: sans }}
            >
      ...
      <div style={{ minHeight: "100vh", background: dark, color: text, fontFamily: serif }}>
        ...
        <div style={{ maxWidth: "800px", margin: "0 auto", padding: "60px 52px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "48px" }}>
            <div style={{ fontSize: "10px", letterSpacing: "0.2em", textTransform: "uppercase", color: `${gold}60`, fontFamily: sans }}>
              Question {currentQ + 1} of {prep.questions.length}
            </div>
            <div style={{ display: "flex", gap: "4px" }}>
              {prep.questions.map((_, i) => (
                <div key={i} style={{ width: "28px", height: "2px", background: i < currentQ ? gold : i === currentQ ? `${gold}80` : "rgba(201,168,76,0.15)", transition: "background 0.3s" }} />
              ))}
            </div>
          </div>
          ...
          <div style={{ marginBottom: "36px" }}>
            <div style={{ display: "flex", gap: "12px", marginBottom: "16px" }}>
              <span style={{ fontSize: "10px", color: `${gold}50`, fontFamily: sans, border: `0.5px solid ${gold}25`, padding: "3px 10px", textTransform: "uppercase", letterSpacing: "0.08em" }}>
                {prep.questions[currentQ]?.type}
              </span>
              <span style={{ fontSize: "10px", color: "rgba(235,229,220,0.2)", fontFamily: sans, border: "0.5px solid rgba(235,229,220,0.1)", padding: "3px 10px", textTransform: "uppercase", letterSpacing: "0.08em" }}>
                {prep.questions[currentQ]?.difficulty}
              </span>
            </div>
            <h2 style={{ fontSize: "clamp(18px, 2.8vw, 28px)", fontWeight: "300", color: text, lineHeight: "1.45", marginBottom: "16px" }}>
              {prep.questions[currentQ]?.question}
            </h2>
            <p style={{ fontSize: "12px", color: "rgba(235,229,220,0.22)", fontFamily: sans, fontStyle: "italic", lineHeight: "1.6" }}>
              Framework: {prep.questions[currentQ]?.idealAnswerFramework} · {prep.questions[currentQ]?.tips}
            </p>
          </div>
          ...
          <div style={{ display: "flex", gap: "12px" }}>
            {currentQ > 0 && (
              <button onClick={() => setCurrentQ(q => q - 1)} style={{ border: `0.5px solid ${gold}20`, background: "none", color: `${gold}50`, padding: "16px 28px", cursor: "pointer", fontSize: "11px", letterSpacing: "0.12em", textTransform: "uppercase", fontFamily: sans }}>
                ← Back
              </button>
            )}
            {currentQ < prep.questions.length - 1 ? (
              <button onClick={() => setCurrentQ(q => q + 1)} style={{ flex: 1, background: `${gold}20`, color: gold, border: `0.5px solid ${gold}30`, padding: "16px", cursor: "pointer", fontSize: "11px", letterSpacing: "0.16em", textTransform: "uppercase", fontFamily: sans }}>
                Next →
              </button>
            ) : (
              <button onClick={handleDebrief} style={{ flex: 1, background: gold, color: dark, border: "none", padding: "16px", cursor: "pointer", fontSize: "11px", letterSpacing: "0.16em", textTransform: "uppercase", fontFamily: sans }}>
                Get my debrief →
              </button>
            )}
          </div>
      ...
      <div style={{ minHeight: "100vh", background: dark, color: text, fontFamily: serif }}>
        ...
        <div style={{ maxWidth: "800px", margin: "0 auto", padding: "60px 52px" }}>
          ...
          <div style={{ textAlign: "center", marginBottom: "52px" }}>
            <div style={{ fontSize: "10px", letterSpacing: "0.24em", textTransform: "uppercase", color: `${blue}60`, marginBottom: "16px", fontFamily: sans }}>
              {vapiConnecting ? "Connecting..." : vapiActive ? "Interview in progress" : "Ready to begin"}
            </div>
            <div style={{ fontSize: "clamp(36px, 5vw, 60px)", fontWeight: "200", color: blue, marginBottom: "8px" }}>
              {prep.interviewerPersona.name}
            </div>
            <div style={{ fontSize: "13px", color: "rgba(235,229,220,0.3)", fontFamily: sans }}>
              {prep.interviewerPersona.title}
            </div>
          ...
          <div style={{ border: `0.5px solid ${blue}20`, padding: "28px 32px", marginBottom: "32px" }}>
            <div style={{ fontSize: "10px", letterSpacing: "0.14em", textTransform: "uppercase", color: `${blue}50`, marginBottom: "12px", fontFamily: sans }}>
              Question {currentQ + 1} of {prep.questions.length}
            </div>
            <p style={{ fontSize: "16px", fontWeight: "300", color: text, lineHeight: "1.6", margin: 0 }}>
              {prep.questions[currentQ]?.question}
            </p>
          </div>
          ...
          {/* Live transcript */}
          {vapiTranscript && (
            <div style={{ marginBottom: "32px", padding: "20px 28px", background: "rgba(107,140,255,0.05)" }}>
              <div style={{ fontSize: "10px", letterSpacing: "0.14em", textTransform: "uppercase", color: `${blue}50`, marginBottom: "8px", fontFamily: sans }}>Your answer (live)</div>
              <p style={{ fontSize: "13px", color: "rgba(235,229,220,0.5)", fontFamily: sans, lineHeight: "1.7", margin: 0, fontStyle: "italic" }}>
                "{vapiTranscript}"
              </p>
            </div>
          )}
          ...
          <div style={{ display: "flex", gap: "12px" }}>
            {currentQ < prep.questions.length - 1 ? (
              <button onClick={() => setCurrentQ(q => q + 1)} style={{ flex: 1, background: `${blue}20`, color: blue, border: `0.5px solid ${blue}30`, padding: "16px", cursor: "pointer", fontSize: "11px", letterSpacing: "0.16em", textTransform: "uppercase", fontFamily: sans }}>
                Next question →
              </button>
            ) : (
              <button onClick={handleDebrief} style={{ flex: 1, background: gold, color: dark, border: "none", padding: "16px", cursor: "pointer", fontSize: "11px", letterSpacing: "0.16em", textTransform: "uppercase", fontFamily: sans }}>
      ...
      <div style={{ minHeight: "100vh", background: "#020202", color: text, fontFamily: serif, display: "grid", gridTemplateColumns: "1fr 1fr", height: "100vh", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", background: "#000", borderRight: "0.5px solid rgba(91,158,122,0.15)" }}>
          ...
          <div style={{ position: "absolute", top: "20px", left: "20px", right: "20px" }}>
            <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
              {[
                { label: "Eye contact", value: Math.round(behaviorData.eyeContactScore) },
                { label: "Confidence", value: Math.round(behaviorData.confidenceScore) },
                { label: "Pace", value: Math.round(behaviorData.paceScore) },
              ].map((item, i) => (
          ...
          {behaviorData.fillerCount > 0 && (
            <div style={{ position: "absolute", bottom: "20px", left: "20px", background: "rgba(0,0,0,0.8)", border: "0.5px solid rgba(176,112,112,0.3)", padding: "8px 14px" }}>
              <div style={{ fontSize: "9px", color: "#B07070", fontFamily: sans, letterSpacing: "0.1em", textTransform: "uppercase" }}>Filler words detected</div>
              <div style={{ fontSize: "13px", color: "#B07070", fontFamily: sans }}>{behaviorData.fillerCount} · {behaviorData.fillerWords.join(", ")}</div>
            </div>
          )}
          ...
          <div style={{ position: "absolute", bottom: "20px", right: "20px" }}>
            <button
              onClick={speechRecognitionActive ? stopSpeechRecognition : startSpeechRecognition}
              style={{
                background: speechRecognitionActive ? "rgba(91,158,122,0.3)" : "rgba(0,0,0,0.7)",
                border: `0.5px solid ${speechRecognitionActive ? "#5B9E7A" : "rgba(255,255,255,0.2)"}`,
                color: speechRecognitionActive ? "#5B9E7A" : "rgba(255,255,255,0.4)",
                padding: "8px 16px", cursor: "pointer", fontSize: "11px", fontFamily: sans, letterSpacing: "0.08em", textTransform: "uppercase",
              }}
            >
              {speechRecognitionActive ? "● Recording" : "Start recording"}
            </button>
          </div>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", background: "#030202", overflow: "auto" }}>
          ...
          <div style={{ padding: "28px", borderBottom: "0.5px solid rgba(91,158,122,0.08)", flexShrink: 0 }}>
            <div style={{ fontSize: "11px", letterSpacing: "0.12em", textTransform: "uppercase", color: "#5B9E7A60", marginBottom: "8px", fontFamily: sans }}>
              {prep.interviewerPersona.name} · {prep.interviewerPersona.title}
            </div>
            <p style={{ fontSize: "16px", fontWeight: "300", color: text, lineHeight: "1.6", margin: 0 }}>
              {prep.questions[currentQ]?.question}
            </p>
            <p style={{ fontSize: "11px", color: "rgba(235,229,220,0.2)", fontFamily: sans, marginTop: "10px", fontStyle: "italic" }}>
              {prep.questions[currentQ]?.tips}
            </p>
          </div>

          {/* Live transcript */}
          <div style={{ padding: "20px 28px", flex: 1 }}>
            <div style={{ fontSize: "9px", letterSpacing: "0.14em", textTransform: "uppercase", color: "rgba(91,158,122,0.4)", marginBottom: "10px", fontFamily: sans }}>
              {speechRecognitionActive ? "Listening..." : "Your answer (type or speak)"}
            </div>
            <textarea
              placeholder="Speak your answer or type here..."
              value={answers[currentQ] || liveTranscript || ""}
              onChange={e => { const a = [...answers]; a[currentQ] = e.target.value; setAnswers(a); }}
              style={{ width: "100%", background: "transparent", border: "none", color: "rgba(235,229,220,0.6)", fontSize: "14px", fontFamily: sans, fontWeight: "300", outline: "none", resize: "none", lineHeight: "1.8", minHeight: "120px" }}
            />
          </div>
          ...
          <div style={{ padding: "20px 28px", borderTop: "0.5px solid rgba(91,158,122,0.08)", display: "flex", gap: "10px", flexShrink: 0 }}>
            {currentQ > 0 && (
              <button onClick={() => setCurrentQ(q => q - 1)} style={{ border: "0.5px solid rgba(91,158,122,0.2)", background: "none", color: "rgba(91,158,122,0.5)", padding: "12px 20px", cursor: "pointer", fontSize: "11px", letterSpacing: "0.1em", textTransform: "uppercase", fontFamily: sans }}>←</button>
            )}
            {currentQ < prep.questions.length - 1 ? (
              <button onClick={() => { setCurrentQ(q => q + 1); setLiveTranscript(""); }} style={{ flex: 1, background: "rgba(91,158,122,0.15)", color: "#5B9E7A", border: "0.5px solid rgba(91,158,122,0.25)", padding: "12px", cursor: "pointer", fontSize: "11px", letterSpacing: "0.14em", textTransform: "uppercase", fontFamily: sans }}>
                Next →
              </button>
            ) : (
              <button onClick={handleDebrief} style={{ flex: 1, background: gold, color: dark, border: "none", padding: "12px", cursor: "pointer", fontSize: "11px", letterSpacing: "0.14em", textTransform: "uppercase", fontFamily: sans }}>
                End & get debrief →
              </button>
            )}
          </div>
      ...
      <div style={{ minHeight: "100vh", background: bg, color: text, fontFamily: serif }}>
        ...
        <div style={{ maxWidth: "900px", margin: "0 auto", padding: "60px 52px" }}>
          ...
          <div style={{ borderBottom: "0.5px solid rgba(201,168,76,0.08)", paddingBottom: "48px", marginBottom: "48px" }}>
            <div style={{ fontSize: "10px", letterSpacing: "0.24em", textTransform: "uppercase", color: "rgba(201,168,76,0.45)", marginBottom: "20px", fontFamily: sans }}>
              Interview debrief · {mode === "written" ? "Written" : mode === "voice" ? "Voice" : "Live Simulation"}
            </div>
            <div style={{ display: "flex", alignItems: "flex-end", gap: "20px", marginBottom: "16px" }}>
              <div style={{ fontSize: "clamp(64px, 10vw, 96px)", fontWeight: "200", color: getScoreColor(debrief.overallScore), lineHeight: "1" }}>
                {debrief.overallScore}
              </div>
              <div style={{ fontSize: "20px", color: "rgba(235,229,220,0.2)", marginBottom: "12px" }}>/100</div>
              <div style={{ marginBottom: "12px", padding: "6px 16px", border: `0.5px solid ${getHiringColor(debrief.hiringDecision)}40`, color: getHiringColor(debrief.hiringDecision), fontSize: "11px", letterSpacing: "0.1em", textTransform: "uppercase", fontFamily: sans }}>
                {debrief.hiringDecision}
              </div>
            </div>
            <p style={{ fontSize: "16px", color: "rgba(235,229,220,0.55)", fontFamily: sans, fontWeight: "300", lineHeight: "1.7", margin: 0, maxWidth: "680px" }}>
              {debrief.verdict}
            </p>
          </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: "1px", background: "rgba(201,168,76,0.07)", marginBottom: "48px" }}>
              {Object.entries(debrief.readinessScore).map(([key, val], i) => (
                <div key={i} style={{ background: bg, padding: "24px 20px" }}>
                  ...
                  <div style={{ height: "2px", background: "rgba(235,229,220,0.05)" }}>
                    <div style={{ height: "100%", width: `${val}%`, background: getScoreColor(val as number) }} />
                  </div>
                </div>
              ))}
            </div>
            ...
            <div style={{ borderBottom: "0.5px solid rgba(201,168,76,0.08)", paddingBottom: "40px", marginBottom: "40px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "1px", background: "rgba(91,158,122,0.08)", marginBottom: "20px" }}>
                {[
                  { label: "Eye contact", value: Math.round(behaviorData.eyeContactScore) },
                  { label: "Confidence", value: Math.round(behaviorData.confidenceScore) },
                  { label: "Speaking pace", value: Math.round(behaviorData.paceScore) },
                ].map((item, i) => (
              ...
              {behaviorData.fillerCount > 0 && (
                <div style={{ padding: "16px 24px", border: "0.5px solid rgba(176,112,112,0.2)" }}>
                  <div style={{ fontSize: "11px", color: "#B07070", fontFamily: sans, marginBottom: "6px" }}>
                    {behaviorData.fillerCount} filler words detected
                  </div>
                  <div style={{ fontSize: "12px", color: "rgba(235,229,220,0.3)", fontFamily: sans }}>
                    Words to watch: {behaviorData.fillerWords.join(", ")}
                  </div>
                </div>
              )}
            </div>
          ...
          {/* Question breakdown */}
          {debrief.questionBreakdown?.length > 0 && (
            <div style={{ borderBottom: "0.5px solid rgba(201,168,76,0.08)", paddingBottom: "40px", marginBottom: "40px" }}>
              <div style={{ fontSize: "10px", letterSpacing: "0.22em", textTransform: "uppercase", color: "rgba(201,168,76,0.45)", marginBottom: "24px", fontFamily: sans }}>Question by question</div>
                ...
                {debrief.questionBreakdown.map((q, i) => (
                  <div key={i} style={{ background: bg, padding: "20px 28px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "10px" }}>
                      <div style={{ fontSize: "13px", fontWeight: "300", color: text, flex: 1, paddingRight: "16px", lineHeight: "1.5" }}>{q.question}</div>
                      <div style={{ fontSize: "16px", color: getScoreColor(q.score), fontFamily: sans, flexShrink: 0 }}>{q.score}</div>
                    </div>
                    <div style={{ fontSize: "12px", color: "rgba(235,229,220,0.3)", fontFamily: sans, marginBottom: "6px" }}>{q.whatTheyDid}</div>
                    <div style={{ fontSize: "12px", color: gold, fontFamily: sans, opacity: 0.6 }}>Better: {q.tip}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          {debrief.encouragement && (
            <div style={{ padding: "32px 40px", border: "0.5px solid rgba(201,168,76,0.15)", marginBottom: "40px" }}>
              <p style={{ fontSize: "16px", color: "rgba(235,229,220,0.6)", fontStyle: "italic", lineHeight: "1.85", margin: "0 0 16px", fontFamily: serif }}>
                "{debrief.encouragement}"
              </p>
          ...
          <div style={{ display: "flex", gap: "16px" }}>
            <button onClick={resetAll} style={{ border: "0.5px solid rgba(235,229,220,0.12)", background: "none", color: "rgba(235,229,220,0.4)", padding: "16px 32px", cursor: "pointer", fontSize: "11px", letterSpacing: "0.12em", textTransform: "uppercase", fontFamily: sans }}>
              Practice again
            </button>
