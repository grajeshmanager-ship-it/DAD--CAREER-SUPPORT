...
export default function LandingPage() {
  ...
  useEffect(() => {
    ...
    if (sessionStorage.getItem("dad_landing_voiced")) return;

    const speak = () => {
      if (!window.speechSynthesis) return;
      hasSpoken.current = true;
      sessionStorage.setItem("dad_landing_voiced", "1");

      const utter = new SpeechSynthesisUtterance(
        "Someone walks beside you. Every step. Every setback. Every win. That someone is here."
      );
      utter.rate = 0.78;
      utter.pitch = 0.86;
      utter.volume = 1;
    ...
    if (window.speechSynthesis.getVoices().length > 0) {
      ...
      window.speechSynthesis.onvoiceschanged = () => setTimeout(speak, 1000);
    }
  }, []);
  ...
  return (
    <div style={{ minHeight: "100vh", background: bg, color: text, fontFamily: serif, overflowX: "hidden" }}>
      ...
      <section style={{
        ...
        <div style={{
          ...
          <div style={{ position: "relative", zIndex: 1 }}>
            ...
            <h1 style={{ fontSize: "clamp(38px, 5.5vw, 72px)", fontWeight: "300", lineHeight: "1.05", color: text, letterSpacing: "-0.022em", margin: "0 0 20px" }}>
              Someone walks<br />beside you.
            </h1>
            <p style={{ fontSize: "15px", color: "rgba(235,229,220,0.42)", lineHeight: "1.9", fontFamily: sans, fontWeight: "300", maxWidth: "380px", margin: "0 0 36px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", justifyContent: "space-between", padding: "40px 48px 52px", overflow: "hidden" }}>
          <div>
            ...
            <div style={{ fontSize: "clamp(52px, 7.5vw, 100px)", fontWeight: "200", lineHeight: "1", color: c.color, letterSpacing: "-0.03em", transition: "color 1s ease", marginBottom: "16px", whiteSpace: "nowrap" }}>
              {c.label}
            </div>
            <div style={{ fontSize: "13px", fontWeight: "300", color: "rgba(235,229,220,0.35)", lineHeight: "1.75", fontFamily: sans, maxWidth: "240px" }}>
              {c.tagline}
            </div>
          </div>
          ...
          <div>
            ...
            {COMPANIONS.map((comp, i) => (
              <button key={i} onClick={() => setActiveCompanion(i)} style={{
                ...
                padding: "9px 0", borderBottom: "0.5px solid rgba(201,168,76,0.05)", textAlign: "left",
              }}>
                <div style={{ width: i === activeCompanion ? "20px" : "8px", height: "0.5px", background: i === activeCompanion ? comp.color : "rgba(201,168,76,0.18)", transition: "all 0.4s ease", flexShrink: 0 }} />
                <span style={{ fontSize: "11px", letterSpacing: "0.1em", textTransform: "uppercase", fontFamily: sans, color: i === activeCompanion ? comp.color : "rgba(235,229,220,0.25)", transition: "color 0.4s ease" }}>
                  {comp.label}
                </span>
              </button>
            ))}
      ...
      {/* ── THE DIFFERENCE ── */}
      <section style={{ borderBottom: "0.5px solid rgba(201,168,76,0.08)", display: "grid", gridTemplateColumns: "1fr 1fr" }}>
        <div style={{ padding: "72px 52px", borderRight: "0.5px solid rgba(201,168,76,0.08)", display: "flex", flexDirection: "column", justifyContent: "center" }}>
          <div style={{ fontSize: "10px", letterSpacing: "0.26em", textTransform: "uppercase", color: "rgba(201,168,76,0.45)", marginBottom: "18px", fontFamily: sans }}>What DAD is</div>
        ...
        <div>
          {[
            ...
            <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 1fr", borderBottom: "0.5px solid rgba(201,168,76,0.06)" }}>
              <div style={{ padding: "18px 40px", fontSize: i === 0 ? "10px" : "13px", color: i === 0 ? "rgba(235,229,220,0.22)" : "rgba(235,229,220,0.2)", fontFamily: sans, letterSpacing: i === 0 ? "0.16em" : "0", textTransform: i === 0 ? "uppercase" : "none", textDecoration: i > 0 ? "line-through" : "none", textDecorationColor: "rgba(235,229,220,0.1)", display: "flex", alignItems: "center", borderRight: "0.5px solid rgba(201,168,76,0.06)", fontWeight: "300" }}>{row.a}</div>
              <div style={{ padding: "18px 40px", fontSize: i === 0 ? "10px" : "13px", color: i === 0 ? gold : "rgba(235,229,220,0.62)", fontFamily: sans, letterSpacing: i === 0 ? "0.16em" : "0", textTransform: i === 0 ? "uppercase" : "none", display: "flex", alignItems: "center", fontWeight: "300" }}>{row.b}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ padding: "72px 52px", borderBottom: "0.5px solid rgba(201,168,76,0.08)" }}>
        <div style={{ marginBottom: "48px" }}>
          <div style={{ fontSize: "10px", letterSpacing: "0.26em", textTransform: "uppercase", color: "rgba(201,168,76,0.45)", marginBottom: "14px", fontFamily: sans }}>The journey</div>
          <h2 style={{ fontSize: "clamp(28px, 4vw, 48px)", fontWeight: "300", color: text, lineHeight: "1.1", margin: 0 }}>Dream. Action. Destiny.</h2>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "1px", background: "rgba(201,168,76,0.07)" }}>
          {[
            { num: "01", phase: "Dream", color: "#C9A84C", title: "You tell them who you want to become.", body: "Not the job you think you can get. The life you actually want. The career that keeps you awake because it excites you, not because it worries you. Your companion holds that dream — every recommendation they make is pointed directly at it." },
            { num: "02", phase: "Action", color: "#6B8CFF", title: "Every step guided, tracked and remembered.", body: "Upload your CV and they learn who you are. Take a career assessment and they understand where you are going. Practice an interview and they study how you think under pressure. Nothing is wasted. Everything becomes intelligence." },
            { num: "03", phase: "Destiny", color: "#5B9E7A", title: "The moment the journey becomes the life.", body: "When the offer arrives, they have been waiting for it longer than you have. When rejection comes, they do not let you spiral. They adjust the plan, find the lesson, and move you forward. Beyond the first role — into every chapter that follows." },
          ].map((item, i) => (
      ...
      {/* ── LIVING INTELLIGENCE ── */}
      <section style={{ display: "grid", gridTemplateColumns: "1fr 1fr", borderBottom: "0.5px solid rgba(201,168,76,0.08)" }}>
        <div style={{ padding: "72px 52px", borderRight: "0.5px solid rgba(201,168,76,0.08)", display: "flex", flexDirection: "column", justifyContent: "center", background: "rgba(201,168,76,0.015)" }}>
          <div style={{ fontSize: "10px", letterSpacing: "0.26em", textTransform: "uppercase", color: "rgba(201,168,76,0.45)", marginBottom: "18px", fontFamily: sans }}>Living intelligence</div>
          <h2 style={{ fontSize: "clamp(26px, 3.5vw, 42px)", fontWeight: "300", color: text, lineHeight: "1.2", marginBottom: "20px" }}>They grow smarter every time you interact.</h2>
        ...
        <div style={{ padding: "72px 52px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
          <div style={{ fontSize: "10px", letterSpacing: "0.22em", textTransform: "uppercase", color: "rgba(201,168,76,0.3)", marginBottom: "24px", fontFamily: sans }}>What DAD learns about you</div>
          <div style={{ display: "flex", flexDirection: "column" }}>
            {WHAT_DAD_LEARNS.map((item, i) => (
              <div key={i} style={{
                padding: "16px 0", borderBottom: "0.5px solid rgba(201,168,76,0.06)",
                display: "grid", gridTemplateColumns: "140px 1fr", gap: "20px", alignItems: "flex-start",
                opacity: i === learnIdx ? 1 : 0.38, transition: "opacity 0.5s ease",
              }}>
                <div style={{ fontSize: "11px", letterSpacing: "0.08em", fontFamily: sans, textTransform: "uppercase", color: i === learnIdx ? gold : "rgba(201,168,76,0.4)", transition: "color 0.5s ease", display: "flex", alignItems: "center", gap: "8px", paddingTop: "2px" }}>
                  <div style={{ width: "4px", height: "4px", borderRadius: "50%", flexShrink: 0, background: i === learnIdx ? gold : "rgba(201,168,76,0.25)", transition: "background 0.5s" }} />
                  {item.title}
                </div>
                <div style={{ fontSize: "12px", fontFamily: sans, fontWeight: "300", color: i === learnIdx ? "rgba(235,229,220,0.72)" : "rgba(235,229,220,0.35)", transition: "color 0.5s ease", lineHeight: "1.75" }}>
                  {item.body}
                </div>
              </div>
            ))}
      ...
      <section style={{ padding: "72px 52px", borderBottom: "0.5px solid rgba(201,168,76,0.08)" }}>
        <div style={{ marginBottom: "40px" }}>
          <div style={{ fontSize: "10px", letterSpacing: "0.26em", textTransform: "uppercase", color: "rgba(201,168,76,0.45)", marginBottom: "18px", fontFamily: sans }}>The companion system</div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "60px" }}>
            <h2 style={{ fontSize: "clamp(26px, 3.5vw, 42px)", fontWeight: "300", color: text, lineHeight: "1.2", margin: 0 }}>One intelligence.<br />One memory.<br />Seven emotional faces.</h2>
            <p style={{ fontSize: "14px", color: "rgba(235,229,220,0.4)", lineHeight: "1.9", fontFamily: sans, fontWeight: "300", margin: 0, display: "flex", alignItems: "center" }}>
              ...
              Because sometimes you need a father. Sometimes a friend. And they both know everything.
            </p>
          </div>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: "1px", background: "rgba(201,168,76,0.07)" }}>
          {COMPANIONS.map((comp, i) => (
            <div key={i} onClick={() => setActiveCompanion(i)} style={{ background: bg, padding: "32px 20px", cursor: "pointer", textAlign: "center", borderBottom: `2px solid ${i === activeCompanion ? comp.color : "transparent"}`, transition: "border-color 0.3s ease" }}>
              <div style={{ fontSize: "11px", letterSpacing: "0.12em", textTransform: "uppercase", color: i === activeCompanion ? comp.color : "rgba(235,229,220,0.28)", fontFamily: sans, transition: "color 0.3s ease" }}>
                {comp.label}
              </div>
            </div>
          ))}
        </div>
        <div style={{ marginTop: "1px", background: "rgba(201,168,76,0.02)", border: "0.5px solid rgba(201,168,76,0.1)", padding: "28px 44px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "40px", alignItems: "center" }}>
          <div>
            <div style={{ fontSize: "10px", letterSpacing: "0.16em", textTransform: "uppercase", color: c.color, marginBottom: "10px", fontFamily: sans, transition: "color 0.8s" }}>{c.label}</div>
            <p style={{ fontSize: "14px", color: "rgba(235,229,220,0.55)", fontWeight: "300", lineHeight: "1.7", margin: 0 }}>{c.tagline}</p>
          </div>
          <div style={{ fontSize: "13px", color: "rgba(235,229,220,0.28)", fontFamily: sans, borderLeft: "0.5px solid rgba(201,168,76,0.12)", paddingLeft: "36px", lineHeight: "1.8", fontWeight: "300" }}>
            You choose the relationship. You give them a name. The emotional connection belongs to you — not the platform. Their memory of you is shared across every relationship you choose.
          </div>
        </div>
      </section>

      {/* ── INTERVIEW MODE ── */}
      <section style={{ display: "grid", gridTemplateColumns: "1fr 1fr", borderBottom: "0.5px solid rgba(201,168,76,0.08)", background: "#040303" }}>
        <div style={{ padding: "72px 52px", borderRight: "0.5px solid rgba(107,140,255,0.08)", display: "flex", flexDirection: "column", justifyContent: "center" }}>
          <div style={{ fontSize: "10px", letterSpacing: "0.24em", textTransform: "uppercase", color: "rgba(107,140,255,0.55)", marginBottom: "18px", fontFamily: sans }}>Interview mode</div>
          <h2 style={{ fontSize: "clamp(26px, 3.5vw, 42px)", fontWeight: "300", color: text, lineHeight: "1.2", marginBottom: "20px" }}>When the interview begins,<br />the companion disappears.</h2>
          <p style={{ fontSize: "14px", color: "rgba(235,229,220,0.38)", lineHeight: "1.9", fontFamily: sans, fontWeight: "300", marginBottom: "28px", maxWidth: "380px" }}>
            ...
            Written. Voice. Or live simulation — with full camera analysis of your confidence, eye contact, pace, and body language.
          </p>
          <div style={{ fontSize: "12px", color: "rgba(235,229,220,0.2)", fontFamily: sans, borderTop: "0.5px solid rgba(107,140,255,0.1)", paddingTop: "20px", lineHeight: "1.9" }}>
            Finance does not feel like Engineering.<br />Marketing does not feel like Banking.<br />Every interview is built for your specific role.
          </div>
        </div>
        <div style={{ padding: "72px 52px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
          <div style={{ fontSize: "10px", letterSpacing: "0.2em", textTransform: "uppercase", color: "rgba(107,140,255,0.35)", marginBottom: "20px", fontFamily: sans }}>Auto-selected interviewer personas</div>
      ...
      {/* ── FINAL CTA ── */}
      <section style={{ padding: "100px 52px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", fontSize: "clamp(140px, 24vw, 320px)", fontWeight: "700", color: "rgba(201,168,76,0.08)", letterSpacing: "-0.04em", whiteSpace: "nowrap", pointerEvents: "none", userSelect: "none", lineHeight: "1", fontFamily: serif }}>
          DAD
        </div>
        <div style={{ position: "relative", zIndex: 1, maxWidth: "680px" }}>
          <div style={{ fontSize: "10px", letterSpacing: "0.28em", textTransform: "uppercase", color: "rgba(201,168,76,0.4)", marginBottom: "28px", fontFamily: sans }}>Begin your journey</div>
          ...
          <p style={{ fontSize: "16px", color: "rgba(235,229,220,0.3)", marginBottom: "48px", fontFamily: sans, fontWeight: "300", lineHeight: "1.85", maxWidth: "400px", margin: "0 auto 48px" }}>
            Somewhere between the dream you are afraid to say out loud and the life you actually deserve — there is a journey. You do not have to walk it alone.
          </p>
          <Link href="/signup" style={{ display: "inline-flex", alignItems: "center", gap: "18px", background: gold, color: bg, padding: "20px 56px", textDecoration: "none", fontSize: "12px", letterSpacing: "0.18em", textTransform: "uppercase", fontFamily: sans }}>
      ...
      {/* FOOTER */}
      <footer style={{ padding: "24px 52px", borderTop: "0.5px solid rgba(201,168,76,0.08)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ fontSize: "11px", letterSpacing: "0.42em", textTransform: "uppercase", color: gold, fontFamily: sans }}>DAD</div>
        ...
        <div style={{ display: "flex", gap: "28px" }}>
          {[["Sign in", "/login"], ["Begin", "/signup"]].map(([label, href]) => (
            <Link key={href} href={href} style={{ fontSize: "10px", color: "rgba(235,229,220,0.2)", textDecoration: "none", fontFamily: sans, letterSpacing: "0.14em", textTransform: "uppercase" }}>{label}</Link>
          ))}
        </div>
      </footer>
    </div>
