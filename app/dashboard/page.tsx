...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data: analysisData } = await supabase.from("analyses").select("*").eq("user_id", user.id).order("created_at", { ascending: false }).limit(5);
  ...
  const companionEmoji = profile?.companion_type ? COMPANION_EMOJIS[profile.companion_type] : null;
  const companionName = profile?.companion_name || null;
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container px-4 md:px-6 max-w-6xl mx-auto py-12">
        <div className="mb-10">
          ...
          {companionName ? (
            <p className="text-muted-foreground text-lg">{companionEmoji} {companionName} is here and ready to help. What do you want to work on today?</p>
          ) : (
            <p className="text-muted-foreground text-lg">DAD is here and ready to help. What do you want to work on today?</p>
          )}
          {profile && (
            <div className="flex items-center gap-2 mt-3 flex-wrap">
              ...
              {companionName && (
                <span className="text-xs bg-secondary text-secondary-foreground rounded-full px-3 py-1">
                  {companionEmoji} Guided by {companionName}
                </span>
              )}
            </div>
          )}
        ...
        <div className="grid md:grid-cols-2 gap-4 mb-10">
          <Link href="/resume">
            <Card className="p-6 border border-border hover:border-primary/50 transition-all duration-200 cursor-pointer group bg-card/50 hover:bg-card h-full">
              ...
              <p className="text-sm text-muted-foreground mb-4">Upload your CV and DAD will tell you exactly what recruiters see — skills gaps, ATS score, salary range</p>
              <div className="flex items-center text-primary text-sm font-medium">Upload resume <ChevronRight className="w-4 h-4 ml-1" /></div>
            </Card>
          </Link>
          ...
          <Link href="/career">
            <Card className="p-6 border border-border hover:border-primary/50 transition-all duration-200 cursor-pointer group bg-card/50 hover:bg-card h-full">
              ...
              <p className="text-sm text-muted-foreground mb-4">Fresh graduate or no experience? Answer a few questions and get a personalised career roadmap, skill plan and job matches</p>
              <div className="flex items-center text-primary text-sm font-medium">Start assessment <ChevronRight className="w-4 h-4 ml-1" /></div>
            </Card>
          </Link>
          ...
          <Link href="/interview">
            <Card className="p-6 border border-border hover:border-primary/50 transition-all duration-200 cursor-pointer group bg-card/50 hover:bg-card h-full relative overflow-hidden">
              ...
              <p className="text-sm text-muted-foreground mb-4">Paste a job description — DAD interviews you like a real recruiter and gives honest feedback with a prep guide</p>
              <div className="flex items-center text-primary text-sm font-medium">Start interview <ChevronRight className="w-4 h-4 ml-1" /></div>
            </Card>
          </Link>
          ...
          <Link href="/voice">
            <Card className="p-6 border border-border hover:border-primary/50 transition-all duration-200 cursor-pointer group bg-card/50 hover:bg-card h-full">
              ...
              <p className="text-sm text-muted-foreground mb-4">Have a real voice conversation — career advice, interview prep, or just talk through what's on your mind</p>
              <div className="flex items-center text-primary text-sm font-medium">Start talking <ChevronRight className="w-4 h-4 ml-1" /></div>
            </Card>
          </Link>
        </div>
              ...
              {analyses.map((a) => (
                <Card key={a.id} className="p-4 border border-border bg-card/50 flex items-center justify-between">
                  <div>
                    ...
                    {a.skill_path && <p className="text-xs text-muted-foreground mt-0.5">{a.skill_path}</p>}
        ...
        {analyses.length === 0 && (
          <Card className="p-8 border border-dashed border-border text-center bg-card/30">
            ...
            <p className="text-muted-foreground">No sessions yet — pick one of the options above and DAD will get to work!</p>
          </Card>
        )}
      </main>
    </div>
