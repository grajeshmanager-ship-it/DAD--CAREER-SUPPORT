    ...
    <div style={{ minHeight: "100vh", background: bg, color: text, fontFamily: serif }}>
      <nav style={{ padding: "18px 52px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "0.5px solid rgba(201,168,76,0.08)", position: "sticky", top: 0, zIndex: 100, background: "rgba(7,6,6,0.97)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
          {nudges.length > 0 && <div style={{ width: "20px", height: "20px", borderRadius: "50%", background: gold, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "10px", color: dark, fontFamily: sans }}>{nudges.length}</div>}
                    ...
                    return (
                      <div key={i} onClick={() => setSelectedMatch(isSelected ? null : match)}
                        ...
                        <div style={{ display: "flex", flexWrap: "wrap", gap: "4px", marginBottom: "8px" }}>
                          {match.tech_stack?.slice(0, 4).map((t, j) => <span key={j} style={{ fontSize: "10px", color: "rgba(235,229,220,0.4)", fontFamily: sans, border: "0.5px solid rgba(235,229,220,0.08)", padding: "2px 8px" }}>{t}</span>)}
                        </div>
                  ...
                  return (
                    <div style={{ background: dark, padding: "28px", borderLeft: "0.5px solid rgba(201,168,76,0.08)" }}>
                      ...
                      <div style={{ marginBottom: "20px" }}>
                        ...
                        <div style={{ fontSize: "11px", color: sc, fontFamily: sans }}>{STAGE_LABELS[selectedMatch.career_stage]}</div>
                      </div>
                      ...
                      <div style={{ marginBottom: "20px" }}>
                        ...
                        <div style={{ display: "flex", flexWrap: "wrap", gap: "4px" }}>
                          {selectedMatch.tech_stack?.map((t, i) => <span key={i} style={{ fontSize: "11px", color: "rgba(235,229,220,0.5)", fontFamily: sans, border: "0.5px solid rgba(235,229,220,0.1)", padding: "3px 10px" }}>{t}</span>)}
                        </div>
                      </div>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "20px", padding: "12px 0", borderTop: "0.5px solid rgba(235,229,220,0.04)" }}>
                        ...
                        <div style={{ fontSize: "24px", fontWeight: "200", color: sc }}>{selectedMatch.score}%</div>
                      </div>
                      ...
                      <p style={{ fontSize: "10px", color: "rgba(235,229,220,0.15)", fontFamily: sans, textAlign: "center", marginTop: "10px", lineHeight: "1.6" }}>Names revealed only when both accept.</p>
                    </div>
                  );
                })()}
              </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: selectedConnection ? "1fr 420px" : "1fr", gap: "1px", background: "rgba(201,168,76,0.06)" }}>
            <div style={{ background: bg }}>
              {connections.length === 0 ? (
                ...
                const op = getOtherProfile(conn);
                const on = getOtherName(conn);
                  ...
                  <div key={i} onClick={async () => { setSelectedConnection(isSel ? null : conn); if (!isSel) await loadMessages(conn.id); }}
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "14px" }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: "11px", color: "rgba(235,229,220,0.2)", fontFamily: sans, marginTop: "2px" }}>{op?.tech_stack?.slice(0, 3).join(" · ")}</div>
                      </div>
            ...
            {selectedConnection && (() => {
              ...
              return (
                <div style={{ background: dark, display: "flex", flexDirection: "column", height: "calc(100vh - 300px)", minHeight: "500px", borderLeft: "0.5px solid rgba(201,168,76,0.08)" }}>
                  <div style={{ padding: "16px 20px", borderBottom: `0.5px solid ${sc}10`, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <div>
                      <div style={{ fontSize: "14px", color: text }}>{on || "Anonymous Developer"}</div>
                      <div style={{ fontSize: "10px", color: sc, fontFamily: sans }}>{STAGE_LABELS[op?.career_stage]}</div>
                    </div>
        ...
        {tab === "pending" && (
          <div style={{ background: bg }}>
            {pending.length === 0 ? (
              ...
              const sc = STAGE_COLORS[conn.buddy_profiles?.career_stage] || gold;
              return (
                <div key={i} style={{ padding: "24px 28px", borderBottom: "0.5px solid rgba(201,168,76,0.05)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "20px" }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "14px", marginBottom: "10px" }}>
                      <div style={{ width: "40px", height: "40px", borderRadius: "50%", background: `${sc}20`, border: `0.5px solid ${sc}40`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "14px", color: sc, flexShrink: 0 }}>?</div>
                      <div>
                        <div style={{ fontSize: "14px", color: text, marginBottom: "3px" }}>Anonymous Developer</div>
                        <div style={{ fontSize: "10px", color: sc, fontFamily: sans }}>{STAGE_LABELS[conn.buddy_profiles?.career_stage] || "Unknown"}</div>
                      </div>
                      <div style={{ fontSize: "20px", fontWeight: "200", color: sc }}>{conn.match_score}%</div>
                    </div>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: "4px", marginBottom: "8px" }}>
