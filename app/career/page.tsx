...
export default function CareerPage() {
  ...
  const allAnswered = questions
    .filter(q => !["concerns", "breakReason", "applied", "whyLeaving"].includes(q.key))
    .every(q => form[q.key]?.trim());
  ...
  if (loading) {
    return (
      <div style={{ minHeight: "100vh", background: bg, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "24px" }}>
        ...
        <div style={{ fontSize: "11px", letterSpacing: "0.22em", textTransform: "uppercase", color: "rgba(201,168,76,0.5)", fontFamily: sans }}>Understanding who you are...</div>
    ...
    <div style={{ minHeight: "100vh", background: bg, color: text, fontFamily: serif }}>
      ...
      {!result ? (
        <div style={{ maxWidth: "800px", margin: "0 auto", padding: "80px 52px" }}>
          <div style={{ marginBottom: "64px" }}>
            ...
            <p style={{ fontSize: "15px", color: "rgba(235,229,220,0.4)", lineHeight: "1.9", fontFamily: sans, fontWeight: "300", maxWidth: "480px", margin: 0 }}>
              Answer honestly. This is your companion's first real attempt to understand you.
              The output will not be a score. It will be understanding.
              "Based on everything you have told me — here is who I think you are."
            </p>
          </div>
          ...
          {!situation ? (
            <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "1px", background: "rgba(201,168,76,0.07)" }}>
                {SITUATIONS.map((s, i) => (
                  <div key={i} onClick={() => setSituation(s.id)}
                    style={{ background: bg, padding: "24px 32px", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between", transition: "background 0.2s" }}
            ...
            <div>
              ...
              <div style={{ display: "flex", flexDirection: "column" }}>
                {questions.map((q, i) => (
                  <div key={i} style={{ borderBottom: "0.5px solid rgba(201,168,76,0.08)", paddingBottom: "4px", marginBottom: "36px" }}>
                    ...
                    {q.type === "input" ? (
                      <input type="text" placeholder={q.placeholder} value={form[q.key] || ""} onChange={e => update(q.key, e.target.value)}
                        style={{ width: "100%", background: "transparent", border: "none", color: text, fontSize: "15px", fontFamily: sans, fontWeight: "300", padding: "4px 0", outline: "none" }} />
                    ) : (
                      <textarea placeholder={q.placeholder} value={form[q.key] || ""} onChange={e => update(q.key, e.target.value)} rows={3}
                        style={{ width: "100%", background: "transparent", border: "none", color: text, fontSize: "15px", fontFamily: sans, fontWeight: "300", padding: "4px 0", outline: "none", resize: "none", lineHeight: "1.7" }} />
                    )}
                  </div>
                ))}
              </div>
              ...
              <button onClick={handleSubmit} disabled={!allAnswered}
                style={{ width: "100%", background: allAnswered ? gold : "rgba(201,168,76,0.15)", color: allAnswered ? bg : "rgba(235,229,220,0.2)", border: "none", padding: "20px", cursor: allAnswered ? "pointer" : "not-allowed", fontSize: "11px", letterSpacing: "0.18em", textTransform: "uppercase", fontFamily: sans, transition: "all 0.3s" }}>
                Build my roadmap →
              </button>
            </div>
          )}
        ...
        <div style={{ maxWidth: "900px", margin: "0 auto", padding: "60px 52px" }}>
          <button onClick={() => { setResult(null); setSituation(null); setForm({}); }}
            style={{ background: "none", border: "none", cursor: "pointer", fontSize: "11px", color: "rgba(235,229,220,0.3)", fontFamily: sans, letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: "48px", padding: 0 }}>
            ← Start over
          </button>
          ...
          <div style={{ borderBottom: "0.5px solid rgba(201,168,76,0.08)", paddingBottom: "48px", marginBottom: "48px" }}>
            ...
            <h2 style={{ fontSize: "clamp(24px, 3.5vw, 40px)", fontWeight: "300", color: gold, lineHeight: "1.2", marginBottom: "20px" }}>
              {result.careerPath}
            </h2>
            <p style={{ fontSize: "16px", color: "rgba(235,229,220,0.6)", lineHeight: "1.9", fontFamily: sans, fontWeight: "300", marginBottom: "20px", maxWidth: "680px" }}>
              {result.whoYouAre}
            </p>
            <p style={{ fontSize: "15px", color: "rgba(235,229,220,0.45)", lineHeight: "1.7", fontFamily: serif, fontStyle: "italic", maxWidth: "600px", margin: 0 }}>
              "{result.headline}"
            </p>
          </div>
          ...
          {result.strongestTraits?.length > 0 && (
            <div style={{ borderBottom: "0.5px solid rgba(201,168,76,0.08)", paddingBottom: "40px", marginBottom: "40px" }}>
              <div style={{ fontSize: "10px", letterSpacing: "0.22em", textTransform: "uppercase", color: "rgba(201,168,76,0.45)", marginBottom: "20px", fontFamily: sans }}>Your strongest traits</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
                {result.strongestTraits.map((trait, i) => (
                  <span key={i} style={{ fontSize: "13px", color: gold, border: `0.5px solid ${gold}50`, padding: "6px 16px", fontFamily: sans }}>
                    {trait}
                  </span>
                ))}
              </div>
                ...
                {result.topRoles.map((role, i) => (
                  <div key={i} style={{ background: bg, padding: "20px 28px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    ...
                    <div style={{ textAlign: "right", flexShrink: 0, marginLeft: "24px" }}>
                      <div style={{ fontSize: "14px", color: gold, fontFamily: sans, marginBottom: "4px" }}>
                        {getCurrencySymbol(role.currency)}{Number(role.salaryMin).toLocaleString()}–{Number(role.salaryMax).toLocaleString()}
                      </div>
                ...
                {result.skillsToLearn.map((s, i) => (
                  <div key={i} style={{ background: bg, padding: "16px 28px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    ...
                    <div style={{ textAlign: "right", flexShrink: 0, marginLeft: "20px" }}>
                      <div style={{ fontSize: "11px", color: s.priority === "high" ? gold : "rgba(235,229,220,0.3)", fontFamily: sans, textTransform: "uppercase", letterSpacing: "0.08em" }}>
                        {s.priority}
                      </div>
          ...
          {result.actionPlan?.length > 0 && (
            <div style={{ borderBottom: "0.5px solid rgba(201,168,76,0.08)", paddingBottom: "40px", marginBottom: "40px" }}>
              <div style={{ fontSize: "10px", letterSpacing: "0.22em", textTransform: "uppercase", color: "rgba(201,168,76,0.45)", marginBottom: "24px", fontFamily: sans }}>Your 12-week action plan</div>
              <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
                {result.actionPlan.map((a, i) => (
                  <div key={i} style={{ display: "grid", gridTemplateColumns: "100px 1fr", gap: "24px", alignItems: "flex-start" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "1px", background: "rgba(201,168,76,0.06)" }}>
                {result.courses.map((c, i) => (
                  <div key={i} style={{ background: bg, padding: "18px 28px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    ...
                    {c.free && (
                      <span style={{ fontSize: "10px", color: "#5B9E7A", border: "0.5px solid #5B9E7A40", padding: "3px 10px", fontFamily: sans, letterSpacing: "0.1em", textTransform: "uppercase", flexShrink: 0, marginLeft: "16px" }}>Free</span>
                    )}
                  </div>
                ))}
              </div>
          ...
          {result.encouragement && (
            <div style={{ padding: "36px 44px", border: "0.5px solid rgba(201,168,76,0.15)", marginBottom: "40px" }}>
